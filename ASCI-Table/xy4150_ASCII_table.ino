...
// pinPlusMinus = HIGH ... Carriage to the right or paper backwards
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (povel.length() > 0) {
      ...
      //mr(lftBorder, hght - topBorder);
      mr(lftBorder, 0);
      drawText(" !\"#$%&'", chCX, 0, 0, chCY, charSpace);
      mr(chCX * -8 * (chW + charSpace), - chCY * (chH + rws));
      drawText("()*+,-./", chCX, 0, 0, chCY, charSpace);
      mr(chCX * -8 * (chW + charSpace), - chCY * (chH + rws));
      drawText("01234567", chCX, 0, 0, chCY, charSpace);
      mr(chCX * -8 * (chW + charSpace), - chCY * (chH + rws));
      drawText("89:;<=>?", chCX, 0, 0, chCY, charSpace);
      mr(chCX * -8 * (chW + charSpace), - chCY * (chH + rws));
      drawText("@ABCDEFG", chCX, 0, 0, chCY, charSpace);
      mr(chCX * -8 * (chW + charSpace), - chCY * (chH + rws));
      drawText("HIJKLMNO", chCX, 0, 0, chCY, charSpace);
      mr(chCX * -8 * (chW + charSpace), - chCY * (chH + rws));
      drawText("PQRSTUVW", chCX, 0, 0, chCY, charSpace);
      mr(chCX * -8 * (chW + charSpace), - chCY * (chH + rws));
      drawText("XYZ[\\]^_", chCX, 0, 0, chCY, charSpace);
      mr(chCX * -8 * (chW + charSpace), - chCY * (chH + rws));
      drawText("`abcdefg", chCX, 0, 0, chCY, charSpace);
      mr(chCX * -8 * (chW + charSpace), - chCY * (chH + rws));
      drawText("hijklmno", chCX, 0, 0, chCY, charSpace);
      ...
      drawText("pqrstuvw", chCX, 0, 0, chCY, charSpace);
      mr(chCX * -8 * (chW + charSpace), - chCY * (chH + rws));
      drawText("xyz{|}~ ", chCX, 0, 0, chCY, charSpace);
      pu();
      mr(lftBorder, 0);
      LineFeed();
      LineFeed();
